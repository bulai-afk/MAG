...
### 6.4. Графики и визуализация данных

Графики являются обязательной частью MVP, а не декоративным дополнением. Их задача — быстро показать динамику, разницу между периодами и вклад каналов без необходимости читать таблицу.

#### Набор графиков MVP

##### 1. Динамика метрики по времени

- тип: line chart;
- ось X: день, неделя или месяц в зависимости от длины периода;
- ось Y: выбранная метрика с единицей измерения;
- основная серия: текущий период;
- дополнительная серия: предыдущий период;
- предыдущий период выравнивается по номеру дня периода, а не по календарной дате;
- переключатель метрики: показы, клики, расходы, конверсии, доход, CTR, CPC, CPA и ROAS;
- derived KPI рассчитываются на backend из агрегированных числителя и знаменателя;
- при одном значении показывается точка и KPI-карточка, а не искусственная линия.

##### 2. Сравнение каналов

- тип: horizontal bar chart;
- категории: source/channel;
- значение: выбранная метрика;
- текущий и предыдущий периоды показываются сгруппированными столбцами;
- сортировка по текущему значению;
- по умолчанию отображаются top 8 каналов, остальные объединяются в «Другие»;
- пользователь может раскрыть полный список в доступной таблице;
- отрицательные значения, если допустимы для метрики, имеют отдельную нулевую линию.

##### 3. Сравнение кампаний

- тип: horizontal bar chart;
- категории: campaign name;
- поддерживается top 10 по расходу, конверсиям или доходу;
- длинные названия сокращаются визуально, но полностью доступны в tooltip и таблице;
- одинаковые названия различаются по external ID или источнику;
- кампании без имени отображаются как «Без названия» с коротким идентификатором;
- фильтр по каналу применяется ко всем связанным KPI и графикам отчёта.

##### 4. Структура расходов и результатов

- тип: 100% stacked horizontal bar;
- отдельные строки для расходов, конверсий и дохода;
- сегменты: каналы;
- график показывает долю канала в общем результате;
- donut/pie chart не использовать по умолчанию, поскольку близкие доли на нём сравниваются хуже;
- при большом числе каналов использовать top 5 и сегмент «Другие».

##### 5. Воронка

- тип: funnel или последовательные horizontal bars;
- этапы MVP: показы → клики → сессии → лиды/конверсии → заказы;
- отображаются только этапы, присутствующие в данных;
- между соседними этапами показывается conversion rate;
- нельзя объединять несовместимые определения конверсий из разных источников;
- если mapping этапов неоднозначен, воронка скрывается и показывается объяснение.

#### Связь KPI и графиков

- клик по KPI-карточке меняет основную метрику графика динамики;
- выбор канала фильтрует кампании и соответствующие narrative facts;
- активный фильтр явно показан над отчётом и может быть сброшен одной кнопкой;
- клиентский snapshot сохраняет выбранную автором метрику и порядок графиков;
- интерактивные фильтры клиента не изменяют опубликованный snapshot и URL без явной необходимости;
- AI получает агрегаты из того же chart dataset, чтобы текст и визуализация не расходились.

#### Контракт данных графика

Backend возвращает готовые к визуализации серии, но не SVG или библиотечные настройки:

```ts
type ChartDataset = {
  id: string;
  type: "timeSeries" | "categoryComparison" | "composition" | "funnel";
  metric: {
    key: string;
    label: string;
...
`null` означает отсутствие данных, а `0` — измеренное нулевое значение. Frontend не пересчитывает KPI и не заменяет `null` нулём.

#### Форматирование

- деньги отображаются в валюте клиента;
- проценты имеют 1–2 знака после запятой;
- большие числа сокращаются только на осях: 1,2 тыс., 3,4 млн;
- tooltip показывает полное значение;
- округление не меняет исходные данные;
- единица измерения указана в заголовке, tooltip и доступной таблице;
- даты форматируются в timezone клиента;
- диапазон оси Y не должен визуально преувеличивать небольшие изменения;
- для bar chart нулевая линия обязательна;
- для line chart ненулевой минимум допустим только при явном обозначении диапазона.

#### Цвет и доступность

- текущий период использует основной брендовый цвет;
- предыдущий период использует нейтральный цвет и пунктир;
- рост/падение дополнительно обозначаются знаком, подписью или иконкой, а не только цветом;
- палитра каналов стабильна внутри одного snapshot;
- контраст соответствует WCAG AA;
- каждый график имеет текстовый заголовок и краткое описание;
- рядом доступна таблица тех же данных;
- tooltip доступен с клавиатуры;
- графики не требуют hover для получения критической информации;
- анимация отключается при `prefers-reduced-motion` и при создании PDF.

#### Адаптивность

- desktop: график и вывод могут располагаться в двух колонках;
- tablet: одна колонка с сохранением легенды;
- mobile: горизонтальные bar charts получают прокрутку или сокращённый top list;
- подписи не накладываются друг на друга;
- легенда переносится на несколько строк;
- минимальная высота touch target — 44 px;
- публичный отчёт не имеет горизонтальной прокрутки всей страницы.

#### PDF и печать

- PDF строится из того же `ChartDataset` и snapshot;
- интерактивные controls скрываются;
- tooltip-значения, необходимые для понимания, дублируются подписями или таблицей;
- фиксируются размеры и шрифты до screenshot/print;
- анимация отключена;
- график не разрывается между страницами;
- легенда печатается вместе с графиком;
- проверяются A4 portrait и landscape для широких сравнений;
- web и PDF проходят visual regression на одинаковом fixture.

#### Состояния данных

- loading: skeleton с фиксированной высотой;
- empty: объяснение, каких данных не хватает;
- partial: предупреждение о неполном периоде;
- error: локальная ошибка графика не скрывает остальной отчёт;
- all-zero: показывается нулевая шкала и пояснение;
- one-point: точка/значение вместо вводящей в заблуждение линии;
- too-many-categories: top N + «Другие» + полная таблица;
- mixed-currency: график блокируется до выбора единой валюты или разделения данных.

#### Производительность

- агрегация выполняется на backend;
- публичная страница получает компактный snapshot без сырых строк;
- число точек одной серии ограничивается разумным лимитом;
- длинные периоды автоматически агрегируются по неделям или месяцам;
- Recharts загружается только на страницах с визуализацией;
- resize не вызывает повторный backend-запрос;
- PDF worker ожидает завершения отрисовки по явному marker, а не фиксированному timeout.

#### Тестирование графиков

- unit: преобразование агрегатов в `ChartDataset`;
- unit: top N, «Другие», null, zero и period alignment;
- integration: chart dataset и KPI используют одинаковые агрегаты;
- component: tooltip, legend, metric switcher и table fallback;
- accessibility: keyboard navigation, labels и contrast;
- visual regression: desktop, mobile и print;
- E2E: фильтр канала, смена метрики, публикация и просмотр snapshot;
- fixture tests: суммы категорий совпадают с общим итогом с учётом округления.

#### После MVP

- scatter plot «стоимость результата ↔ объём конверсий»;
- annotations событий и изменений бюджета;
- forecast и confidence interval;
- cohort и retention charts для подходящих данных;
- пользовательский выбор типа графика;
- сохранённые dashboard presets;
- экспорт изображения;
- cross-client benchmarks только на корректно анонимизированных агрегатах.
